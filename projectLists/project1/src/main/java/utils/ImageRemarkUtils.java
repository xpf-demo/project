...
public class ImageRemarkUtils {
    ...
    // 水印文字字体
    private static Font font = new Font("宋体", Font.BOLD, 15);
    // 水印文字颜色
    ...
    public static void setImageMarkOptions(float alpha, int positionWidth,
        ...
        if (alpha != 0.0f)
            ImageRemarkUtils.alpha = alpha;
        if (positionWidth != 0)
            ImageRemarkUtils.positionWidth = positionWidth;
        if (positionHeight != 0)
            ImageRemarkUtils.positionHeight = positionHeight;
        if (font != null)
            ImageRemarkUtils.font = font;
        if (color != null)
            ImageRemarkUtils.color = color;
    }
    ...
    public static void markImageByText(String logoText, String srcImgPath,
        ...
        try {
            // 1、源图片
            Image srcImg = ImageIO.read(new File(srcImgPath));
            int bgImgWidth =  srcImg.getWidth(null);//背景图片宽度
            int bgImgHeight =  srcImg.getHeight(null);//背景图片长度
            ...
            g.drawString(logoText, bgImgWidth-font.getSize()*4-20, bgImgHeight-font.getSize());
            // 9、释放资源
            g.dispose();
            // 10、生成图片
            os = new FileOutputStream(targerPath);
